...
export async function POST(request: NextRequest) {
  try {
    ...
    // Muse 专门使用 DIFY_PROGRESS_MENTOR_APP_ID，不使用通用的 DIFY_API_KEY
    // 这样确保 Muse 不会误用其他 API key（如 plot brainstorming 的 key）
    const apiKey = DIFY_PROGRESS_MENTOR_APP_ID
    ...
    console.log('Muse (Progress Mentor) API Config:', {
      ...
      stage,
      action,
      note: 'Muse always uses DIFY_PROGRESS_MENTOR_APP_ID, not DIFY_API_KEY or other app IDs',
    })
    
    if (!apiKey || !DIFY_PROGRESS_MENTOR_APP_ID) {
      return NextResponse.json(
        { error: 'DIFY_PROGRESS_MENTOR_APP_ID not configured for Muse' },
        { status: 500 }
      )
    }
