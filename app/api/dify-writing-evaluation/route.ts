...
const DIFY_WRITING_EVAL_APP_ID = 'app-1aoSk7aJ0xjaYGlVVIQrxvQe' // Muse Writing Evaluation
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 使用Muse的app ID作为API key
    const apiKey = DIFY_WRITING_EVAL_APP_ID
    if (!apiKey) {
      return NextResponse.json(
        { error: 'DIFY_WRITING_EVAL_APP_ID not configured' },
        { status: 500 }
      )
    }
    
    console.log('Muse Writing Evaluation API - Using app ID:', apiKey)
